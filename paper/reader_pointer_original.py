...
def input_data(N_filename, T_filename):
    ...
    with open(N_filename, "rb") as f:
        ...
        save = pickle.load(f)
        train_dataN = save["train_data"]
        # test_dataN = save['test_data']
    ...
    with open(T_filename, "rb") as f:
        ...
        save = pickle.load(f)
        train_dataT = save["train_data"]
        # test_dataT = save['test_data']
    ...
    return train_dataN, None, vocab_sizeN, train_dataT, None, vocab_sizeT, attn_size
...
def data_producer(
    ...
    with tf.name_scope(name):
        ...
        if verbose:
            ...
            print(
                "long_lineT_len: %d, empty: %.4f, unk: %.4f, location: %.4f, eof: %.4f, word (except Empty): %.4f"
                % (
                    long_lineT_len,
                    float(empty_cnt) / long_lineT_len,
                    float(unk_cnt) / long_lineT_len,
                    float(l_cnt) / long_lineT_len,
                    float(eof_cnt) / long_lineT_len,
                    float(w_cnt) / long_lineT_len,
                )
            )
            print("the most common 5 of cnt_T", cnt_T.most_common(5))
            print(
                "print verbose information and take %.2fs\n"
        ...
        tf_dataN = tf.convert_to_tensor(
            long_lineN_truncated, name="raw_dataN", dtype=tf.int32
        )
        tf_dataT_x = tf.convert_to_tensor(
            long_lineT_truncated_x, name="raw_dataT_x", dtype=tf.int32
        )
        tf_dataT_y = tf.convert_to_tensor(
            long_lineT_truncated_y, name="raw_dataT_y", dtype=tf.int32
        )
        ...
        dataN = tf.reshape(
            tf_dataN[0 : batch_size * batch_len], [batch_size, batch_len]
        )
        dataT_x = tf.reshape(
            tf_dataT_x[0 : batch_size * batch_len], [batch_size, batch_len]
        )
        dataT_y = tf.reshape(
            tf_dataT_y[0 : batch_size * batch_len], [batch_size, batch_len]
        )

        epoch_size = (batch_len - 1) // num_steps
        assert (
            epoch_size > 0
        ), f"epoch_size = {epoch_size}, batch_len = {batch_len}, num_steps = {num_steps}"
        ...
        )  # need to assert all values in x[a,:,1] are the same
        yN = tf.strided_slice(
            dataN, [0, i * num_steps + 1], [batch_size, (i + 1) * num_steps + 1]
        )
        print(f"yN.shape = {yN.shape}")
        yN.set_shape([batch_size, num_steps])

        xT = tf.strided_slice(
            dataT_x, [0, i * num_steps], [batch_size, (i + 1) * num_steps]
        )
        xT.set_shape(
            [batch_size, num_steps]
        )  # need to assert all values in x[a,:,1] are the same
        yT = tf.strided_slice(
            dataT_y, [0, i * num_steps + 1], [batch_size, (i + 1) * num_steps + 1]
        )
        yT.set_shape([batch_size, num_steps])

        eof_indicator = tf.equal(
            xN[:, num_steps - 1], tf.constant([eof_N_id] * batch_size)
        )
        print(
            "Finish preparing input producer and takes %.2fs"
            % (time.time() - start_time)
        )
...
if __name__ == "__main__":
    ...
    loss_condition = tf.greater(
        labels, tf.constant(value=eof_id, dtype=tf.int32, shape=labels.shape)
    )
    fetches = {
        "labels": labels,
