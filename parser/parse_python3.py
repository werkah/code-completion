    ...
    def traverse(node):
        ...
        # Process children.
            ...
            for item in node.items:
                withitem_node = traverse(item)
                children.append(withitem_node)
            children.append(traverse_list(node.body, "body"))
        elif isinstance(node, ast.Match):
            children.append(traverse(node.subject))
            children.append(traverse_list(node.cases, "cases"))
        elif isinstance(node, ast.match_case):
            children.append(traverse(node.pattern))
            children.append(traverse_list(node.body, "body"))
        elif isinstance(node, ast.MatchValue):
            children.append(traverse(node.value))
        elif isinstance(node, ast.MatchSingleton):
            children.append(traverse(node.value))
        elif isinstance(node, ast.MatchAs):
            if node.name:
                children.append(gen_identifier(node.name))
            if node.pattern:
                children.append(traverse(node.pattern))
        elif isinstance(node, ast.MatchClass):
            children.append(traverse(node.cls))
        elif isinstance(node, ast.MatchMapping):
            children.append(traverse_list(node.keys, "keys"))
            children.append(traverse_list(node.patterns, "patterns"))
            ...
            for child in ast.iter_child_nodes(node):
                if (
                    isinstance(child, ast.expr_context)
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open(output_filename, "w", encoding="utf-8") as output_file:
            json.dump(result, output_file, separators=(",", ":"), ensure_ascii=False)
